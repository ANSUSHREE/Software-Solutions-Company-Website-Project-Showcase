...
interface FormData {
  ...
  timezoneSupport?: { name?: string; email?: string; details?: string } | null;
...
const customSoftwareQuestions: Question[] = [
  ...
  },
  {
    ...
    options: [
      ...
      { label: 'Other', value: 'other', description: 'Please specify your market' },
    ],
  },
  {
    id: 3,
  ...
  },
  {
    ...
    options: [
      ...
      { label: 'IoT', value: 'ioT' },
      { label: 'Other', value: 'other', description: 'Please specify platforms' },
    ],
  },
  {
    id: 5,
  ...
  },
  {
    ...
    options: [
      ...
      { label: 'Other', value: 'other', description: 'Please specify technologies' },
    ],
  },
  {
    id: 7,
    ...
    required: true,
    options: [
      { label: 'No, I would not', value: 'no, I wouldnt' },
      { label: 'Yes, with other systems', value: 'other', description: 'Please specify integrations' },
    ],
  },
  {
    id: 8,
    ...
    fields: [
      ...
      },
      {
        ...
        options: [
          ...
          { label: 'Other', value: 'other', description: 'Please specify priorities' }
        ]
      }
    ]
  },
...
const teamAugQuestions: Question[] = [
  {
    ...
    options: [
      ...
      { label: 'Other expert(s)', value: 'other', description: 'Please specify experts' },
    ],
  },
  {
    id: 2,
  ...
  },
  {
    ...
    options: [
      ...
      { label: 'Another software type', value: 'other', description: 'Please specify software type' },
    ],
  },
  {
    id: 8,
...
const CostEstimator: React.FC<CostEstimatorProps> = ({
  ...
  const handleCompositeChange = (parentField: keyof FormData, subField: string, value: string | string[]) => {
    setFormData(prev => ({
      ...prev,
      [parentField]: {
        ...((typeof prev[parentField] === 'object' && prev[parentField] !== null) ? prev[parentField] : {}),
        [subField]: value
      }
    }));
    setError(null);
  ...
  const handleOtherOptionChange = (field: keyof FormData, otherValue: string) => {
    setFormData(prev => ({
      ...prev,
      otherOptions: {
        ...(prev.otherOptions || {}),
  ...
  const isOtherSelected = (field: keyof FormData, value: string) => {
    ...
    if (currentQuestion.required) {
      if (currentQuestion.type === 'composite') {
        for (const field of currentQuestion.fields || []) {
          if (field.required) {
            const parentField = formData[currentQuestion.field] || {};
            if (field.type === 'checkbox') {
              if (!parentField[field.field] || parentField[field.field].length === 0) {
                setError(`Please select at least one option for "${field.label}"`);
                return false;
              }
            } else if (!parentField[field.field]) {
      ...
      else if (currentQuestion.options?.some(opt =>
        opt.value === 'other' &&
        isOtherSelected(currentQuestion.field, opt.value) &&
        !formData.otherOptions?.[currentQuestion.field]
      )) {
        setError("Please specify your answer for 'Other' option");
        return false;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/costestimator", {
        method: "POST",
        headers: {
      ...
      return (
        <div className="space-y-6">
          {question.fields?.map((field) => {
            const parentData = formData[question.field] || {};
            const fieldValue = parentData[field.field] || '';
              ...
              <div key={field.field} className="space-y-2">
                ...
                {field.type === 'text' ? (
                  <input
                    ...
                    onChange={(e) => handleCompositeChange(
                      question.field,
                      field.field,
                      e.target.value
                    )}
                  ...
                  <div className="space-y-2">
                    {field.options?.map(opt => (
                      <div key={opt.value} className="space-y-1">
                        ...
                        {opt.value === 'other' && checkboxValues.includes('other') && (
                          <div className="ml-6 mt-1">
                            <input
                              type="text"
                              placeholder={opt.description || "Please specify"}
                              value={formData.otherOptions?.[`${question.field}_${field.field}`] || ''}
                              onChange={(e) => handleOtherOptionChange(
                                `${question.field}_${field.field}` as keyof FormData,
                                e.target.value
                              )}
                              className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                            />
        ...
        <div className="space-y-3">
          {question.options?.map((opt) => (
            <div key={opt.value} className="space-y-2">
              ...
              {opt.value === 'other' && isOtherSelected(question.field, opt.value) && (
                <div className="ml-8 mt-1">
                  <input
                    type="text"
                    placeholder={opt.description || "Please specify"}
                    value={formData.otherOptions?.[question.field] || ''}
                    onChange={(e) => handleOtherOptionChange(question.field, e.target.value)}
                    className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              )}
            ...
            <div className="md:w-1/3">
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="bg-red-500 text-white px-5 py-2 font-medium transition-all duration-300 hover:bg-opacity-0 hover:text-red-500 hover:shadow-md transform hover:-translate-y-0.5 border border-red-600 inline-flex items-center justify-center w-fit disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            <div className="md:w-1/2">
              ...
              <div className="flex justify-between mt-8">
                ...
                <button
                  ...
                  className="bg-red-500 text-white px-6 py-2 font-medium transition-all duration-300 hover:bg-opacity-0 hover:text-red-500 hover:shadow-md transform hover:-translate-y-0.5 border border-red-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
